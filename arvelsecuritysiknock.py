...
while True:
    ...
    while (flag == 1):
        ...
        if(kocksensordelantero == False):
            print("Se detecto anomalia en la parte delantera - Knock sensor ----> "+str(kocksensordelantero))
            with picamera.PiCamera() as camera:
                camera.resolution = (1280,720)
                camera.capture("/home/pi/Desktop/cookie/newimage.png")
            time.sleep(2)
        if(kocksensortrasero == False):
            print("Se detecto anomalia en la parte trasera - Knock sensor ----> "+str(kocksensortrasero))
            with picamera.PiCamera() as camera:
                camera.resolution = (1280,720)
                camera.capture("/home/pi/Desktop/cookie/newimage.png")
            time.sleep(2)
